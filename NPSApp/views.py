...
def process_reviews(json_source, filtro):
    ...
    def process_review(review):
        ...
        # Comprobar si alguna palabra del filtro está en la reseña
        if any(filtro in review_text_normalizado for filtro in filtros):
...
def analizar_universidad(request, universidad_id):
    ...
    try:
        ...
        # Actualiza el diccionario data para incluir la desviación estándar
        data = {
            'nps': puntuacion[0],
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse(data)  # Devuelve los datos como JSON si la solicitud es AJAX
    except Exception as e:
        print(f"Error durante el procesamiento: {e}")
        traceback.print_exc()  # Esto imprimirá la traza del error en la consola
        raise e  # Re-lanzar el error para que Django maneje la respuesta de error

    return render(request, 'home.html', {'data': data})  # Devuelve la página con los datos si no es una solicitud AJAX